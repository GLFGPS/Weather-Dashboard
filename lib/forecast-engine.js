...
function groupDropsIntoWaves(drops) {
  ...
  for (let i = 1; i < sorted.length; i++) {
    ...
    const daysBetween = Math.round((thisDate - prevDate) / 86400000);
...
export function computeProjection(dateStr, { weather, dmInHome, drops, warmStreak } = {}) {
  const d = new Date(`${dateStr}T00:00:00`);
  ...
  return {
    ...
    weatherCondition,
    weatherMultiplier,
    warmStreakMultiplier: warmStreakMult,
