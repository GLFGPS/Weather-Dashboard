...
const MODEL_VERSION = "live";

const CONFIRMED_PROJECTIONS = {
  "2026-03-25": 279,
  "2026-03-26": 235,
};
...
export async function getProjectionLog({ startDate, endDate } = {}) {
  ...
  query += ` ORDER BY forecast_date ASC, actual_total IS NULL ASC, logged_at ASC;`;

  const result = await dbQuery(query, params);

  return result.rows.map((row) => {
    const override = CONFIRMED_PROJECTIONS[row.date];
    if (override != null) {
      const projTotal = override;
      const error =
        row.actual_total != null ? row.actual_total - projTotal : null;
      const errorPct =
        error != null && projTotal > 0
          ? Math.round(((row.actual_total - projTotal) / projTotal) * 1000) / 10
          : null;
      return {
        ...row,
    ...
    return {
      ...
      error:
        row.actual_total != null
          ? row.actual_total - row.projected_total
          : null,
      error_pct:
        row.actual_total != null && row.projected_total > 0
          ? Math.round(
              ((row.actual_total - row.projected_total) / row.projected_total) *
                1000,
            ) / 10
          : null,
    };
