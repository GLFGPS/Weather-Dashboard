...
export async function getLeadOverview({
  ...
  let selectedYearWeatherByMarket = new Map();
  ...
  for (const targetYear of selectedCompareYears) {
    ...
    const weatherInfo = await getPriorityWeatherByDate(
      ...
      window.end,
      weatherApiKey,
      { includeByMarket: targetYear === selectedYear },
    );
    for (const storage of weatherInfo.storageModes) {
      weatherStorageModes.add(storage);
    }
    if (targetYear === selectedYear && weatherInfo.byMarket) {
      selectedYearWeatherByMarket = weatherInfo.byMarket;
    }
  ...
  const decisionByDate = buildDecisionByDate(daily, goalTracking);
  const marketElasticity = buildMarketWeatherElasticity(daily, selectedYearWeatherByMarket);
