...
const SEASON_END_MM_DD = "12-31";
const SEASON_START_DAY_KEY = 215;
const SEASON_END_DAY_KEY = 1231;
...
async function getDailyLeadRows(startDate, endDate, sourceFilter) {
  ...
  const useDirectMailAggregateFilter = normalizedSource.toLowerCase() === "direct mail";
  ...
  if (useDirectMailAggregateFilter) {
    const result = await dbQuery(
      `
        SELECT
          lead_date::text AS date,
...
async function getPriorityWeatherByDate(
  ...
  for (const marketEntry of priorityMarketNames) {
    const marketName =
      typeof marketEntry === "string"
        ? marketEntry
        : String(marketEntry?.name || "").trim();
    const weatherLocation =
      typeof marketEntry === "string"
        ? marketEntry
        : marketEntry?.weatherQuery || marketName;
...
export async function getLeadOverview({
  ...
  const sourceOptions = [...new Set(["All Sources", "Direct Mail", ...topSources.map((row) => row.source)])];
